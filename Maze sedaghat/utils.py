...
def detect_object( img , x_step = 5 , y_step =  1) :
...
def approx_predict(img , threshold = 150 ):



    for i in range(threshold ,201 , 50) :

        bl = cv2.cvtColor(255*np.uint8(img > threshold)   , cv2.COLOR_BGR2GRAY  )
        cv2.imshow("out" , bl)
